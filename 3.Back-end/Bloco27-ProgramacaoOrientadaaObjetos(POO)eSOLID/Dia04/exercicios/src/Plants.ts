...
const plantsNeedSun = (id: string) =>
  (plant: IPlant): boolean =>
    plant.id === id
    && plant.needsSun
    && (!plant.specialCare || plant.specialCare.waterFrequency > 2);

class Plants {
  private PLANTS_PATH = path.join(
    __dirname,
    'database',
    'plantsData.json',
  );

  private OPS_INFO_PATH = path.join(
    __dirname,
    'database',
    'opsInfo.json',
  );
  ...
  private async updateOpsInfo(incrementAmount = 1): Promise<void> {
    const opsInfoRaw = await fs.readFile(this.OPS_INFO_PATH, {
      encoding: 'utf8',
    });
